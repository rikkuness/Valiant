...
namespace BlueprintReality.MixCast
{
    ...
    public class JPGEncoder
    {
        ...
        static Dictionary<int, QuantTableData> quantTableDict = null;
        QuantTableData currentQuantTable
        {
            get
            {
                if (quantTableDict == null)
                {
                    quantTableDict = new Dictionary<int, QuantTableData>();
                }
                if (!quantTableDict.ContainsKey(currentSf))
                {
                    var table = CreateQuantTables(currentSf);
                    quantTableDict.Add(currentSf, table);
                }
                return quantTableDict[currentSf];
            }
        }

        class QuantTableData
        {
        ...
        private QuantTableData CreateQuantTables(int sf)
        {
            var table = new QuantTableData();
            int i;
            float t;
            int[] YQT = new int[]{
            ...
            for (i = 0; i < 64; i++)
            {
                ...
                table.YTable[ZigZag[i]] = Mathf.RoundToInt(t);
            }
            ...
            for (int row = 0; row < 8; row++)
            {
                ...
                {
                    table.fdtbl_Y[i] = (1.0f / (table.YTable[ZigZag[i]] * aasf[row] * aasf[col] * 8.0f));
                    table.fdtbl_UV[i] = (1.0f / (table.UVTable[ZigZag[i]] * aasf[row] * aasf[col] * 8.0f));
                    i++;
                }
            }
            return table;
        ...
        static BitString[] m_YDC_HT = null;
        BitString[] YDC_HT
        {
            get
            {
                if (m_YDC_HT == null)
                {
                    m_YDC_HT = ComputeHuffmanTbl(std_dc_luminance_nrcodes, std_dc_luminance_values);
                }
                return m_YDC_HT;
            }
        }

        static BitString[] m_UVDC_HT = null;
        BitString[] UVDC_HT
        {
            get
            {
                if (m_UVDC_HT == null)
                {
                    m_UVDC_HT = ComputeHuffmanTbl(std_dc_chrominance_nrcodes, std_dc_chrominance_values);
                }
                return m_UVDC_HT;
            }
        }

        static BitString[] m_YAC_HT = null;
        BitString[] YAC_HT
        {
            get
            {
                if (m_YAC_HT == null)
                {
                    m_YAC_HT = ComputeHuffmanTbl(std_ac_luminance_nrcodes, std_ac_luminance_values);
                }
                return m_YAC_HT;
            }
        }

        static BitString[] m_UVAC_HT = null;
        BitString[] UVAC_HT
        {
            get
            {
                if (m_UVAC_HT == null)
                {
                    m_UVAC_HT = ComputeHuffmanTbl(std_ac_chrominance_nrcodes, std_ac_chrominance_values);
                }
                return m_UVAC_HT;
            }
        }
        ...
        static BitString[] m_bitcode = null;
        BitString[] bitcode
        {
            get
            {
                if (m_bitcode == null || m_category == null)
                {
                    m_bitcode = new BitString[65535];
                    m_category = new int[65535];
                    InitCategoryfloat();
                }
                return m_bitcode;
            }
        ...
        */
        ...
        private void WriteDQT()
        {
            ...
            for (i = 0; i < 64; i++)
            {
                WriteByte((byte)currentQuantTable.YTable[i]);
            }
            WriteByte(1);
            for (i = 0; i < 64; i++)
            {
                WriteByte((byte)currentQuantTable.UVTable[i]);
            }
        }
        ...
        public JPGEncoder(Texture2D texture, float quality, string path)
        {
            ...
            currentSf = (quality < 50) ? (int)(5000 / quality) : (int)(200 - quality * 2);
        ...
        */
        ...
        private void Encode()
        {
            ...
            byteout = new ByteArray(image.width * image.height);
            bytenew = 0;
            bytepos = 7;
            ...
            for (int ypos = 0; ypos < image.height; ypos += 8)
            {
                for (int xpos = 0; xpos < image.width; xpos += 8)
                {
                    ...
                    DCY = ProcessDU(YDU, currentQuantTable.fdtbl_Y, DCY, YDC_HT, YAC_HT);
                    DCU = ProcessDU(UDU, currentQuantTable.fdtbl_UV, DCU, UVDC_HT, UVAC_HT);
                    DCV = ProcessDU(VDU, currentQuantTable.fdtbl_UV, DCV, UVDC_HT, UVAC_HT);
                }
            }
